...
create type if not exists application_status as enum ('found','reviewed','approved','applied','interview','rejected','offer');
create type if not exists lead_status as enum ('discovered','researched','qualified','drafted','approved','contacted','replied','meeting','client','closed');
create type if not exists approval_status as enum ('pending','approved','rejected','skipped','executed','failed');
...
create table if not exists knowledge_documents (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(user_id,source_type,content_hash)
);
...
language sql stable as $$
  select kd.id,kd.source_type,kd.source_id,kd.title,kd.content,kd.metadata,
         1-(kd.embedding <=> query_embedding) as similarity
  from knowledge_documents kd
  where kd.user_id=p_user_id and kd.embedding is not null
