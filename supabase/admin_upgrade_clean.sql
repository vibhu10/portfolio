-- Clean migration for Experience CMS + Contact Inbox
...
insert into public.experiences
  (title, company, location, start_date, end_date, description, skills, visible, sort_order)
select
  ...
  'Feb 2024',
  'Present',
  'Building production applications, APIs, data pipelines, automation and cloud infrastructure.',
  array['Next.js','Node.js','Python','Supabase','Docker']::text[],
  true,
  0
where not exists (
  select 1 from public.experiences
  where lower(title) = lower('Full-Stack Developer')
    and lower(company) = lower('WebNext Technologies')
);
...
select
  ...
  array['React','Node.js','Express.js','MongoDB']::text[],
  true,
  1
where not exists (
  select 1 from public.experiences
  where lower(title) = lower('MERN Stack Developer')
    and lower(company) = lower('ShineDezign')
);
